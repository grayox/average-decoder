// Change Log: v03: Converted order.html to Bootstrap 100% grid system; Separated user contact form form to account.html;
//-------------------------------------- Code.gs --------------------------------------
...
function order_receive(ob){var AR=["yield","volume"],i=AR.length;while(i--){ob[AR[i]]=Number(ob[AR[i]])}ob.email=Session.getUser().getEmail();ob.status="Open";orderFill(ob)}//Handled by orderFill(ob):return LibraryjsUtil.dbParse({verb:"post",project:"taxLienExchange",className:"order",ob:ob})}//ob.price=Number(ob.price);ob.volume=Number(ob.volume);
...
function order_fill   (id){var user=Session.getUser().getEmail();LibraryjsUtil.dbParse({verb:"put",project:"taxLienExchange",className:"order",obid:id,ob:{status:"Pending"  ,counterparty:user}}) // For matched, existing orders. Contrasted with orderFill() which is for unmatched new orders.
...
-------------------------------------- order.html --------------------------------------
<!DOCTYPE html><html class="fuelux">
<head>
    <meta charset="utf-8"/>
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
...
<body>
      <?var user=(Session.getActiveUser().getEmail())?Session.getActiveUser().getEmail():"disabled",arr,mm=orderMatchArr(LibraryjsUtil.dbParse({verb:"get",project:"taxLienExchange",className:"order",query:{email:user,status:"Open"}}).results)
...
<?!=LibraryjsUtil.signInMsg()?>
	<div class="col-xs-3 xcol-xs-offset-3">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">My Pending Orders
                    ...
                    <tbody>
                        <?arr=ar_mp.reverse();i=arr.length;while(i--){var strAlt=(i%2)? "class='alt'": "";//vol=arr[i].volume?LibraryjsUtil.numberWithCommas(arr[i].volume): "",price=arr[i].price ? "$"+LibraryjsUtil.numberWithCommas(arr[i].price ): "" ?>
                            <tr <?!=strAlt?>>
                                ...
                                <td>
                                    <input autocomplete="off" type="checkbox" xchecked onClick="myFunction_orderCancel(<?=arr[i].item?>)" name="cancelOrder" title="Check to cancel order.">
                                </td>
                            </tr>
                            <?}?>
                    </tbody>
        ...
        <div class="panel panel-default">
            <div class="panel-heading">My Closed Orders
                    ...
                    <tbody>
                        <?arr=ar_mc.reverse();i=arr.length;while(i--){var strAlt=(i%2)? "class='alt'": "";//vol=arr[i].volume?LibraryjsUtil.numberWithCommas(arr[i].volume): "",price=arr[i].price ? "$"+LibraryjsUtil.numberWithCommas(arr[i].price ): "" ?>
                            <tr <?!=strAlt?>>
                                ...
                                <!--<td><input autocomplete="off" type="checkbox" xchecked xonClick="alert('A')"  name="fillOrder" title="Check to fill this order. To fill an order means to take the opposite side of the transaction. i.e., Buy if the order is to sell. And vice versa."></td>-->
                            </tr>
                            <?}?>
                    </tbody>
                </table>
    ...
    <div class="col-xs-3 xcol-xs-offset-3">
        <div class="panel xpanel-default  panel-primary xpanel-success">
            <div class="panel-heading">Create New Order
            ...
            <div class="panel-body">
                <form id="order" name="order" class="form form-inline xform-horizontal" role="form">
                    ...
                    <div class="btn-group col-xs-6">
                        <label for="btnSubmitOrder" xhidden>Click</label>
                        <button autocomplete="off" disabled="<?=user?>" type="button" class="btn btn-block btn-primary" role="button" id="btnSubmitOrder" xclass="blue|#428bca" value="Submit Order" onclick="if(<?=user?>=='disabled'){alert('Please login')}else{myFunction_orderCreate();alert('Success!\n\nWe received your order.')}" title="">Submit Order</button>
                    </div>
        ...
        <div class="panel xpanel-default xpanel-primary  panel-success">
            <div class="panel-heading">My Matched Orders
                <!--<span class="glyphicon glyphicon-question-sign"></span> --><img height="15" src="https://lh6.googleusercontent.com/-eY7ym_Fp4BM/VGMjeM1ugiI/AAAAAAAANGI/nCzT6fqLUjM/s800/icon-question.png" title="All open orders that “match” your open orders; i.e., potential counterparties. Check the “fill” box to accept the order and move it to “pending.”">
                <!--<span class="glyphicon glyphicon-user"         ></span>-- ><img height="14" src="https://lh4.googleusercontent.com/-qfsYcPDhFpk/VGMjemXK03I/AAAAAAAANF8/mj2BfzU0e44/s800/icons-plus.png">-->
            </div>
            <div class="panel-body">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        <?arr=ar_mm.reverse();i=arr.length;while(i--){var strAlt=(i%2)? "class='alt'": "";//vol=arr[i].volume?LibraryjsUtil.numberWithCommas(arr[i].volume): "",price=arr[i].price ? "$"+LibraryjsUtil.numberWithCommas(arr[i].price ): "" ?>
                            <tr <?!=strAlt?>>
                                ...
                                <td>
                                    <input autocomplete="off" <?=user?> type="checkbox" xchecked onClick="myFunction_orderFill(<?=arr[i].item?>)" name="fillOrder" title="Check to fill this order. To fill an order means to take the opposite side of the transaction. i.e., Buy if the order is to sell. And vice versa.">
                                </td>
                            </tr>
                            <?}?>
                    </tbody>
                    ...
                    <tbody>
                        <?arr=ar_ao.reverse();i=arr.length;while(i--){var strAlt=(i%2)? "class='alt'": "";//vol=arr[i].volume?LibraryjsUtil.numberWithCommas(arr[i].volume): "",price=arr[i].price ? "$"+LibraryjsUtil.numberWithCommas(arr[i].price ): "" ?>
                            <tr <?!=strAlt?>>
                                ...
                                <td>
                                    <input autocomplete="off" <?=user?> type="checkbox" xchecked onClick="if(<?=user?>=='disabled'){alert('Please login')}else{myFunction_orderFill(<?=arr[i].item?>)}" name="fillOrder" title="Check to fill this order. To fill an order means to take the opposite side of the transaction. i.e., Buy if the order is to sell. And vice versa.">
                                </td>
                            </tr>
                            <?}?>
                    </tbody>
    ...
    <div class="col-xs-3 xcol-xs-offset-3">
        <div class="panel  panel-default xpanel-primary xpanel-success">
            <div class="panel-heading">Market
                <!--<span class="glyphicon glyphicon-question-sign"></span>--><img height="15" src="https://lh6.googleusercontent.com/-eY7ym_Fp4BM/VGMjeM1ugiI/AAAAAAAANGI/nCzT6fqLUjM/s800/icon-question.png" title="Lowest yield open buy order and highest yield open sell order.">
            </div>
            <div class="panel-body">
                <div class="btn-group col-xs-6">
                    <button autocomplete="off" disabled="<?=user?>" type="button" class="btn btn-block btn-success" role="button" id="btnMktBuy" xvalue="Submit Order" onclick="if(<?=user?>=='disabled'){alert('Please login')}else{myFunction_orderFill(<?=mktBuy.item?>)}" title="Clicking this button results in the same action as (i.e, a shortcut to) finding the HIGHEST yielding open SELL order in the “open orders” table, then clicking the “fill” box next to it. In both cases, we will notify your counterparty and move the order to your “pending” list to set you up to close the transaction.">
                        Buy <span class="badge"><?=function(){try{return /*LibraryjsUtil.num2cur*/(/*mp[0]*/mktBuy.price).toFixed(2)}catch(e){Logger.log("Error B2Rcy: "+e.message);return "--"}}()?>%</span>
                    </button>
                </div>
                <div class="btn-group col-xs-6">
                    <button autocomplete="off" disabled="<?=user?>" type="button" class="btn btn-block btn-danger" role="button" id="btnMktSell" xvalue="Submit Order" onclick="if(<?=user?>=='disabled'){alert('Please login')}else{myFunction_orderFill(<?=mktSell.item?>)}" title="Clicking this button results in the same action as (i.e, a shortcut to) finding the LOWEST yielding open BUY order in the “open orders“ table, then clicking the “fill” box next to it. In both cases, we will notify your counterparty and move the order to your “pending” list to set you up to close the transaction.">
                        Sell <span class="badge"><?=function(){try{return /*LibraryjsUtil.num2cur*/(/*mp[1]*/mktSell.price).toFixed(2)}catch(e){Logger.log("Error G2OfN: "+e.message);return "--"}}()?>%</span>
                    </button>
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                        <?arr=ar_ap.reverse();i=arr.length;while(i--){var strAlt=(i%2)? "class='alt'": "";//vol=arr[i].volume?LibraryjsUtil.numberWithCommas(arr[i].volume): "",price=arr[i].price ? "$"+LibraryjsUtil.numberWithCommas(arr[i].price ): "" ?>
                            <tr <?!=strAlt?>>
                                ...
                                <!--<td><input autocomplete="off" <?=user?> type="checkbox" xchecked xonClick="alert('A')"  name="fillOrder" title="Check to fill this order. To fill an order means to take the opposite side of the transaction. i.e., Buy if the order is to sell. And vice versa."></td>-->
                            </tr>
                            <?}?>
                    </tbody>
                </table>
