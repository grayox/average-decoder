...
function orderMatch(ob){ob=ob||{price:10,volume:5,type:"Buy"/*"Sell"*/,name:"Atlas",phone:"2064862010",email:"atlaslive@gmail.com"}//@return{array} all open orders "matching" this order //@param{object} ob: the order that needs matching
    var ar,q={},op=(ob.type=="Buy")?"$lte":"$gte";q[op]=ob.price;ar=LibraryjsUtil.dbParse("get","marketMaker","order",{price:q,type:{"$ne":ob.type}});return ar}//Logger.log(ar)} // Reference: https://www.parse.com/docs/rest#queries
function receiveOrder(ob){var AR=["price","volume"],i=AR.length;while(i--){ob[AR[i]]=Number(ob[AR[i]])}return LibraryjsUtil.dbParse("post","marketMaker","order",ob)}//ob.price=Number(ob.price);ob.volume=Number(ob.volume);
function serveTable(){var r,ob=[],d=new Date(),t=d.getTime(),user=Session.getUser().getEmail(),ar=LibraryjsUtil.dbParse("get","marketMaker","order"/*,{owner:user}*/).results.sort(function(a,b){var x="price";return(a[x]-b[x])}),i=ar.length,out={"totalResultsCount":i+1,"records":[]};while(i--){out.records[i]={};r=ar[i];
	ob[i] = {
		"nolink"                         : {
			...
			,	"price"                  : function(){try{return LibraryjsUtil.num2cur         (r.price )}catch(e){Logger.log(e.message);return "";}}() //
			...
			,	"volume"                 : function(){try{return LibraryjsUtil.numberWithCommas(r.volume)}catch(e){Logger.log(e.message);return "";}}() //
										   }
	,	"link"		                     : {}
	}
...
<style>
    xinput,xselect {color:#000000;xheight:100px;font-size:90%;padding:1px 2px;xborder:1px solid #000000;border-radius:3px;text-align:left}
...
<body><?var uynf="",uync="",uyp="",user=(Session.getActiveUser().getEmail())?Session.getActiveUser().getEmail():null,p=user?("value='"+user+"' disabled"):"placeholder='Your email'"
  ...
  <div style="width:770px;margin:0 auto;xtext-align:left">
    <form id="listing"><!--References: https://developers.google.com/apps-script/guides/html-service-communication#forms https://developers.google.com/apps-script/guides/html-service-communication#user_objects-->
        <fieldset><legend>Place limit order</legend>
            ...
            <select             style="color:#888888;text-align:center" name="type">
	    ...
	    <input type="button" value="Send order" onclick="myFunction()" style="xbackground:#000000;font-size:100%;xcolor:#FFFFFF;xfont-weight:bold;xborder-radius:3px;xpadding:1px 2px;xdisplay:inline-block;width:770px;text-align:center" title=""><!-- Reference: http://stackoverflow.com/questions/5462251/button-width-in-css3 -->
	</form></div>
	<h1>Open orders</h1>
<div class="datagrid" style="margin:auto;width:300px;xwidth:50%">
	...
	<tbody><?var ar=serveTable().records,i=ar.length;while(i--){var strAlt=(i%2)?"class='alt'":"";//vol=ar[i].volume?LibraryjsUtil.numberWithCommas(ar[i].volume):"",price=ar[i].price ?"$"+LibraryjsUtil.numberWithCommas(ar[i].price ):""
         ?><tr <?!=strAlt?>><td title="This is the order made by your counterparty"><?=ar[i].item?></td><td style="text-align:center" title="This is what your counterparty wants to do. If this column says Buy, then to fill the order, you will Sell. And vice versa."><?=ar[i].type?></td><td style="text-align:right" title="The number of shares to be traded"><?=ar[i].volume?></td><td title="The transaction price" style="text-align:right"><?=ar[i].price?></td>
	 	       <td><input type="checkbox" xchecked name="fillOrder" title="Check to fill this order. To fill an order means to take the opposite side of the transaction. i.e., Buy if the order is to sell. And vice versa."></td></tr><?}?>
	</tbody></table></div>
<script>
