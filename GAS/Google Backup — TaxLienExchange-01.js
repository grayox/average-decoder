...
function orderMatch(ob){//ob=ob||{price:10,volume:5,type:"Buy"/*"Sell"*/,name:"Atlas",phone:"2064862010",email:"atlaslive@gmail.com"}//@return{array} all open orders "matching" this order //@param{object} ob: the order that needs matching
    var ar,q={},op=(ob.type=="Buy")?"$lte":"$gte";q[op]=ob.price;ar=LibraryjsUtil.dbParse("get","taxLienExchange","order",{status:"Open",price:q,type:{"$ne":ob.type}});return ar}//Logger.log(ar)} // Reference: https://www.parse.com/docs/rest#queries
function orderFill(ob){//ob=ob||{price:3.6,volume:500,type:"Buy"/*"Sell"*/,name:"Atlas",phone:"2064862010",email:"atlaslive@gmail.com"}//@return{void} "fills" all open orders "matching" this order //@param{object} ob: the order that needs matching
    ...
    while(bal&&i--){                      ar[i].status               = "Pending"              ;
                                          ...
                                          LibraryjsUtil.dbParse("put" ,"taxLienExchange","order",ar[i].objectId,ar[i])
                   }if(bal){ob.volume=bal;LibraryjsUtil.dbParse("post","taxLienExchange","order",ob                  )}}
function receiveOrder(ob){var AR=["price","volume"],i=AR.length;while(i--){ob[AR[i]]=Number(ob[AR[i]])}ob.email=Session.getUser().getEmail();ob.status="Open";return LibraryjsUtil.dbParse("post","taxLienExchange","order",ob)}//ob.price=Number(ob.price);ob.volume=Number(ob.volume);
function serveTable(){var r,ob=[],d=new Date(),t=d.getTime(),user=Session.getUser().getEmail(),ar=LibraryjsUtil.dbParse("get",/*"taxLienExchange"*/"marketMaker","order"/*,{owner:user},{status:"Open"}*/).results.sort(function(a,b){var x="price";return(a[x]-b[x])}),i=ar.length,out={"totalResultsCount":i+1,"records":[]};while(i--){out.records[i]={};r=ar[i];
	ob[i] = {
		"nolink"                         : {
...
-------------------------------------- tables.html --------------------------------------
	<h1>All open orders</h1>
        <div class="datagrid" style="margin:auto;width:300px;xwidth:50%">
            <table class="table table-striped table-hover"><thead><th>Item</th><th>Type</th><th>Vol</th><th>Price</th><!--<th>Fill</th>--></thead><tfoot><tr><td colspan="5"><div id="no-paging" xid="paging">&nbsp;<!--<ul><li><a href="#" ><span>Previous</span></a></li><li><a href="#" class="active"><span>1</span></a></li><li><a href="#"><span>2</span></a></li><li><a href="#"><span>3</span></a></li><li><a href="#"><span>4</span></a></li><li><a href="#"><span>5</span></a></li><li><a href="#"><span>Next</span></a></li></ul>--></div></td></tr></tfoot>
...
-------------------------------------- order.html --------------------------------------
<!DOCTYPE html><html style="text-align:center;font-family:arial;font-size:100%;color:#888888;">
<head>
...
<!--<link rel="stylesheet" href="https://googledrive.com/host/0B1LVOoV_2dFtMzM0LXE2QTU4b1E"><!--Tables-->
    <style>
       ...
       xinput        {color:#000000;xheight:100px;font-size:90%;padding:1px 2px;xborder:1px solid #000000;border-radius:3px;text-align:left}
        input,select {padding:7px 10px;border-radius:5px;xmargin-bottom:3px;xcolor-blue-button-primary:#428bca;xcolor-blue-button-hover:#3071a9;xcolor:#3e0901;text-align:center;}
	    fieldset     {text-align:left;border-radius:3px}
       #yield        {border-radius:0px}
...
<body><?var user=(Session.getActiveUser().getEmail())?Session.getActiveUser().getEmail():null,arr,ar_ao=[],ar_mo=[],ar_ap=[],ar_mp=[],ar_ac=[],ar_mc=[],ar=serveTable().records,i=ar.length;while(i--){
      ...
      <?var uynf="",uync="",uyp="",p=user?("value='"+user+"' disabled"):"placeholder='Your email'"
          ; try{if(user){u=LibraryjsUtil.dbParse("get","taxLienExchange","usera",{email:user}).results[0]}}catch(e){Logger.log(e.message)}try{uynf=u.yNameFull?"value='"+u.yNameFull+"'":""}catch(e){}try{uync=u.yNameComp?"value='"+u.yNameComp+"'":""}catch(e){}try{uyp=u.yPhone?"value='"+u.yPhone+"'":""}catch(e){}?>
    <div style="xwidth:770px;margin:0 auto;xtext-align:left">
    <form id="listing" class="form-inline" role="form"><!--References: https://developers.google.com/apps-script/guides/html-service-communication#forms https://developers.google.com/apps-script/guides/html-service-communication#user_objects-->
        <div class="row">
		<fieldset><legend>Contact</legend>
                  <div class="col-xs-3">
            <input type="text"  xsize="30" id="email"  name="email"  placeholder="Email"   <?!=p?>    title="Your email address. Example: johndoe@example.com"        >
            </div><div class="col-xs-3">
		    <input type="text"  xsize="25" id="name"   name="name"   placeholder="Name"    <?!=uynf?> title="Your full first and last names. Example: John Doe, III"  >
            </div><div class="col-xs-3">
            <input type="text"  xsize="11" id="phone"  name="phone"  placeholder="Phone"   <?!=uyp?>  title="Your company phone number. Example: 555-555-1212"        > <!--<br><input type="text"   style="color:#888888;xheight:100px;font-size:100%;padding:5px;xborder:1px solid #00DD00;border-radius:5px;" size="35" id="phoneEve" name="phoneEve" placeholder="2nd phone" title="Alternate phone number to reach customer. Example: 555-555-1212" > // Reference: http://stackoverflow.com/questions/1994406/set-image-as-submit-button // Works on jsfiddl.net but not GAS -->
  	        </div>
		</fieldset>
		</div>
        <fieldset><legend>Order</legend>
            <select xclass="form-control"  required    name="type"   >
			    <option value=""      >--Type--</option>
			    <option value="Buy"   >  Buy   </option>
                <option value="Sell"  >  Sell  </option>
            </select>
                <input xclass="form-control" type="number"  size="5" id="yield" name="yield"  required  placeholder="Rate"  min="0" max="72" step="0.25" title="What rate of effective yield are you offering (sell) or seeking (buy)?Minimum target effective current yield. (Annual interest accrual divided by maximum bid (purchase price).)"   >
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Trade Volume $
            <input xclass="form-control" type="number"  size="5" id="volume"   name="volume" required  placeholder="Dollars" min="0" step="1" title="Dollar volume you are seeking to trade"  > 
	    <input type="button" class="btn xbtn-block btn-primary" xclass="blue|#428bca" value="Submit" onclick="myFunction();alert('Success!\nWe received your order.');" title="" ><!-- Reference: http://stackoverflow.com/questions/5462251/button-width-in-css3 -->
	    </fieldset>
